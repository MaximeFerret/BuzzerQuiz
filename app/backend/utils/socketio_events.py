...
def on_correct_answer(data):
    ...
    if username in players and "score" in players[username]:
        ...
        emit(
            "correct_answer",
            {"username": username, "score": players[username]["score"]},
            room=room,
            broadcast=True,
        )
...
def on_start_game(data):
    ...
    active_quizzes[room] = {"questions": questions, "current_question": 0}
...
def on_host_join(data):
    ...
    room_players = [
        username for username, data in players.items() if data["room"] == room
    ]
